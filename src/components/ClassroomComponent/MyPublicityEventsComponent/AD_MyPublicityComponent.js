...
class AD_MyPublicityComponent extends Component {
  constructor(props) {
    super(props)
  ...
  approve(event) {
    let scope = this;
    approvePublicity(event, 'AD', this.props.user)
  ...
  flagReject(event, message, mode) {
    let scope = this;
    flagRejectPublicity(event, message, mode, 'AD', this.props.user)
    const {myArrx} = scope.state
    delete myArrx[event.key]
    scope.setState({myArrx})
  ...
  componentDidMount() {
    ...
    this.setState({fetching: true})
        var scope = this;
        firebaseDB.ref('events/publicity/').orderByChild('AD_appr').equalTo('pending').on('value',
        function(snapshot) {
...
export default connect(mapStateToProps)(AD_MyPublicityComponent)