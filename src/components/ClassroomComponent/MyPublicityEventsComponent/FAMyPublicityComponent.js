...
class FAMyPublicityComponent extends Component {
  constructor (props) {
    super(props)
  ...
  approve (event) {
    let scope = this
    approvePublicity(event, 'FA', this.props.user)
  ...
  flagReject (event, message, mode) {
    let scope = this
    flagRejectPublicity(event, message, mode, 'FA', this.props.user)
    const { myArrx } = scope.state
    delete myArrx[event.key]
    scope.setState({ myArrx })
...
export default connect(mapStateToProps)(FAMyPublicityComponent)
